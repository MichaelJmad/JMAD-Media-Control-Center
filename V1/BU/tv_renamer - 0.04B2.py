...
class OrganizeDialog(tk.Toplevel):
    ...
    def _build_ui(self):
        ...
        tb.Separator(action_frame, orient="horizontal").pack(fill="x", pady=20)
        tb.Button(action_frame, text="Undo", command=self.undo_local_change, bootstyle="outline-warning").pack(pady=5, fill="x")
        tb.Button(action_frame, text="Redo", command=self.redo_local_change, bootstyle="outline-info").pack(pady=5, fill="x")
    ...
    def undo_local_change(self):
        messagebox.showinfo("Undo", "Local Undo is not yet implemented.")

    def redo_local_change(self):
        messagebox.showinfo("Redo", "Local Redo is not yet implemented.")
